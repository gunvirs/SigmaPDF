...
:
    ...
    ranGen(label(time(0))),
    // From here...
    turbLenghtRatio_(0),//    turbLenghtRatio_(0.010),
    diameter_(0),    // Diameter ( i.e. Characteristic Lenght
    umean_(0),
    turbIntensity_(0),
    readExp_(false),
    //stoppete,
...
:
    ...
    ranGen(label(time(0))),
    // Turb Lenght	scalar turbLenghtRatio_;
    turbLenghtRatio_(ptf.turbLenghtRatio_),
    // Diameter ( i.e. Characteristic Lenght
    diameter_(ptf.diameter_),
    umean_(ptf.umean_),
    turbIntensity_(ptf.turbIntensity_),
    readExp_(ptf.readExp_),
    //stoppete
...
:
    ...
    ranGen(label(time(0))),
    // Turb Lenght	scalar turbLenghtRatio_;
    turbLenghtRatio_(dict.lookupOrDefault<scalar>("turbLenghtRatio", 0.4)),
    diameter_(readScalar(dict.lookup("diameter"))),
    umean_(readScalar(dict.lookup("umean"))),
    turbIntensity_(dict.lookupOrDefault<scalar>("turbIntensity", 0.01)),
    readExp_(dict.lookupOrDefault<bool>("readExp", false)),
    //stoppete
    cleanRestart_(false),
    isInitialized_(false),
    isRestart_(false),
...
{
    //Info << "Foam::klein dict constructor" << endl;
...
void Foam::klein::fixGrid()
{
	label a = NY_;
	label b = NZ_;
	scalar c = dy_;
	NZ_ = Pstream::nProcs(); 
	Info << "Virtual grid spacing dy: " << dy_ << " dz: " << dz_ << " a " << a << " b " << b << " C " << c <<  endl;
	NY_ = Pstream::nProcs();
	dy_=LY_/(NY_+1);
	dz_=LZ_/(NZ_+1);
	Info << "Virtual grid spacing dy: " << dy_ << " dz: " << dz_   << endl;
	dy_=((c*(a+1))/(dy_*(NY_+1)))*LY_/(NY_+1);
	dz_=((c*(b+1))/(dz_*(NZ_+1)))*LZ_/(NZ_+1);
	Info << "Virtual grid spacing dy: " << dy_ << " dz: " << dz_ << endl;
	Info << " ratio "  << dy_/c  << " ratio "  << dz_/c << endl;
	indicesPerProc_ = floor(scalar(NY_*NZ_)/(Pstream::nProcs()));
	scalar rest=scalar(NY_*NZ_)-(indicesPerProc_*(Pstream::nProcs()));
        Info << "AFTER: indicesPerProc: "  << indicesPerProc_ << " rest: " << rest << endl;
}

// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
void Foam::klein::initData()
{
    ...
    if(Pstream::master())
    {
        indicesPerProc_ = floor(scalar(NY_*NZ_)/(Pstream::nProcs()));
        label rest =0;
        rest=scalar(NY_*NZ_)-(indicesPerProc_*(Pstream::nProcs()));
        Info << "NOW: indicesPerProc: " << indicesPerProc_ << " rest: " << rest << endl;
        if(rest>0)
        {
            indicesPerProc_ = floor(scalar(NY_*NZ_))/(Pstream::nProcs()-1);
            rest=scalar(NY_*NZ_)-(indicesPerProc_*(Pstream::nProcs()-1));
		if(rest>indicesPerProc_)
		{
			fixGrid();
		}
		else
	...
	yindices_[I] =floor(I/NZ_);
	zindices_[I]=(I-yindices_[I]*NZ_);
    }
	...
	if(readExp_)	//AGGIUNTO QUESTO, nel caso questo e' vero, allora legge i dati esternamente come ora avviene di default
	{
	if(Pstream::master())
	{
	...
	mapperIV_Ptr_.reset //mapper from inlet data to virtual grid --> Vale lo stesso discorso fatto
	(
	new pointToPointPlanarInterpolation
	(
	samplePointsField,
	...
	{
		LxField_.setSize(ReStress.size());
		LyField_.setSize(virualGridPoints_.size());
		LzField_.setSize(virualGridPoints_.size());
		UMean =umean_*vector(1,0,0);

		ReStress.replace(symmTensor::XX, pow(turbIntensity_*UMean.component(0),2));

		ReStress.replace(symmTensor::YY, pow(turbIntensity_*UMean.component(0),2));

		ReStress.replace(symmTensor::ZZ, pow(turbIntensity_*UMean.component(0),2));

		ReStress.replace(symmTensor::XY, scalar(0));
		ReStress.replace(symmTensor::XZ, scalar(0));
		ReStress.replace(symmTensor::YZ, scalar(0));


		//Info << "Fatto Re Stress "<< endl;
		samplePointsField = virualGridPoints_;
		//Info << "Fatto smp points  "<< endl;
		LxField_ = diameter_*turbLenghtRatio_*vector(1,1,1);Info << "Lx  "<< endl;
		LyField_ = diameter_*turbLenghtRatio_*vector(1,1,1);Info << "Ly  "<< endl;
		LzField_ = diameter_*turbLenghtRatio_*vector(1,1,1);Info << "Lz  "<< endl;
    ...
    forAll(ReStress, I)
    {
        if ((ReStress[I].xx() < 0) || (ReStress[I].yy() < 0) || (ReStress[I].zz() < 0))
            FatalErrorIn("klein::")<< "Some RMS in input file R are negative"<<abort(FatalError);
    }


    Lund_.replace(tensor::XX, sqrt(ReStress.component(symmTensor::XX))+1e-8);
    ...
    Lund_.replace(tensor::YY, sqrt(ReStress.component(symmTensor::YY)-sqr(Lund_.component(tensor::YX)))+1e-8);
    ...
    mapperVP_Ptr_.reset//mapper from virtual grid to patch.
    (
        new pointToPointPlanarInterpolation
        (
            virualGridPoints_,
    ...
    Info << "converted x-length scale to timestep-scale (using frozen turbulence assumption)" << endl;
...
Foam::scalar Foam::klein::getRandomNumber()
{
...
void Foam::klein::getRandomField()
{
    if(Pstream::master())
    {
        //Fill random fields
...
void Foam::klein::getFilterCoeff_New(scalarList& b_x, label NLX_x)
{
...
//1d filter coeff, new format
void Foam::klein::get2DFilterCoeff_New(scalarList& filter,label NLY_x, label NLZ_x)
{
    scalarList by_x;
    scalarList bz_x;
    ...
    forAll(virtualFilteredFieldProc_[Pstream::myProcNo()],subI)
    {
        if(doSpatialCorr)
        {
            ...
            for (int ii=0;ii<(nfK_*NLyField_[I].component(0)+1);ii++)
            {
                ...
                sumTmp_u+=sumProd(rnd,filt);//sumProd definito in scalarField.C in line 93 e poi in FieldM.H in linea 385. Dovrebbe essere += rnd(i-esimo)*filt(i-esimo) cioe' motiplico il rand number iesimo per il filtro calcolato iesimo. Non e' chiaro perche' questa procedura. 
                //sumTmp_u+=rnd[0];
            }
		Info << "1" << endl;
            for (int ii=0;ii<(nfK_*NLyField_[I].component(1)+1);ii++)
            {
                ...
                sumTmp_v+=sumProd(rnd,filt);//sumProd definito in scalarField.C in line 93 e poi in FieldM.H in linea 385. Dovrebbe essere += rnd(i-esimo)*filt(i-esimo) cioe' motiplico il rand number iesimo per il filtro calcolato iesimo. Non e' chiaro perche' questa procedura. 
...
{	
    ...
    else
    {
        ...
        Info << "klein: preparing first time step " << endl;
        initData();
        getRandomField();
	Info <<"Randomizzato" << endl;
        spatialCorr();
	Info <<"Spatialcorrelizzato" << endl;
        interpolFluct();
	Info <<"Interpolato" << endl;
        uFluctTemporal_old = uFluctFiltered;
 	Info <<"Salvato per time step dopo" << endl;
   }
}

inline int Foam::klein::get1DIndex(int x, int y, int yMax)
{
    //helper function to convert 2d array index into 1d list index. Array is converted in y direction into list.
    return x*yMax + y;
}

//inline void Foam::klein::get2DIndex(int I, int& x, int& y, int yMax)
//{
...
void Foam::klein::write(Ostream& os) const
{
	...
	//
    os.writeKeyword("turbLenghtRatio") << turbLenghtRatio_<< token::END_STATEMENT << nl;//turbLenght/InletCharacteristicLenght
    os.writeKeyword("diameter") << diameter_<< token::END_STATEMENT << nl;
    os.writeKeyword("umean") << umean_<< token::END_STATEMENT << nl;
    os.writeKeyword("turbIntensity") << turbIntensity_<< token::END_STATEMENT << nl;//Per ora presa come Urms/Umean
    os.writeKeyword("readExp") << readExp_ << token::END_STATEMENT << nl;
	//
...
void Foam::klein::autoMap(const fvPatchFieldMapper& m)
{
...
void Foam::klein::rmap(const fvPatchField<vector>& ptf, const labelList& addr)
{
    fixedValueFvPatchField<vector>::rmap(ptf, addr);

    const klein& tiptf = refCast<const klein>(ptf);
